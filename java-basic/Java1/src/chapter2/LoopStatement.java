...
public class LoopStatement {
	...
	public static void main(String[] args) {
		...
		// for(반복에 사용될 변수 초기화; 반복조건; 변수 증감){ 반복할 코드 블록 }
		...
		System.out.println("========================================");
		//foreach 문
		//배열을 반복 돌면서 요소를 하나씩 복사하여 해당 요소의 데이터를 사용
		// for(배열 요소의 데이터타입 배열 요소의 변수명: 반복할 배열){코드블록}
		...
		// foreach 는 읽기전용으로 값을 사용하는 경우에만 적합하다.
		// 배열의 요소 값을 변경하는 작업은 불가능 하다.
	...
	//	while(inputNumber > 0) {
		...
		while(true) {
			//continue;
			//반복문을 해당위치에서 강제로 조건 검사로 넘겨버리는 키워드
			if(number % 2 == 0) {
				number++;
				continue;
			}
