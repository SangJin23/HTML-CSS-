...
//생성은 new연산자를 이용해서 생성 할 수 있음
//배열(참조변수)는 실제 데이터가 저장되는 것이 아니라
...
public class Array {
	...
	System.out.println("=====================================");
		...
		// 5. 문자 찾기 - 문자열.indexOf(찾을 문자) , 문자열.lastIndexOf(찾을문자)
		int hyphenIndex = identityNumber.indexOf("-");
		System.out.println(hyphenIndex);
		
		int zeroLastIndex = identityNumber.lastIndexOf("0");
		System.out.println(zeroLastIndex);
		
		// 6. 부분 문자열 - 문자열.substring(시작 인덱스, 끝 인덱스);
		String birthString = identityNumber.substring(0,14);
		System.out.println(birthString);
		
		System.out.println("=====================================");
		// 기본형 데이터 타입과 참조형 데이터 타입의 비교
		// 기본형 데이터 타입은 직접 그 데이터를 가지고 있음
		// 참조형 데이터 타입은 데이터가 저장된 주소를 있음
		int basicType1 = 10;
