...
public class Operator {
	...
	public static void main(String[] args) {
		...
		// 증감 연산자: 단항 연산자, 변수에만 사용가능,피연산자에 대한 연산자의 위치에 따라 연산 순서가 다름
		// ++(증가 연산자): 피연산자(변수)를 1증가 시
		...
		//일반대입 연산자 = : 좌항(변수)에 우항의 연산 결과를 대입
		int number3 = 10;
		
		// 복합 대입 연산자 : 특정 연산 수행결과를 좌항에 다시 할당함
		// += , -= ,*= , /= , %= 등이 존재함.
		
		//+= : 좌항에 우항을 더한 값을 좌항에 대입 (좌항 = 좌황 + 우항)
		number3 += 3; // number3 = number3 + 3;
		System.out.println(number3);
		
		//-= : 좌항에 우항을 뺀 값을 좌항에 대입 (좌항 = 좌황 - 우항)
		number3 -= 9;
		System.out.println(number3);
		...
		//&&( 논리 AND 연산자) : 좌항과 우항의 표현식이 모두 true 일 때 true를 반환
		...
		// ||(논리 Or 연산자) : 좌항과 우항 중 하나라도 TRUE 이면 true 르ㅜㄹ 반환
		result = trueFlag || trueFlag;
		System.out.println(result);
		result = trueFlag || falseFlag;
		System.out.println(result);
		...
		// ! (논리 NOT 연산자) : 우항의 논리값을 반전 시킴 ,true면 false ,false면 true
		result = !trueFlag;
		System.out.println(result);
		System.out.println("======================================");
		//삼항 연산자
		...
		// 논리 표현식 ? true일때 반환될 연산 결과 : false 반환될 연산 결과
		int score = 75;
		String grade = 
				score > 90 ? "A" : 
				score > 80 ? "B" : 	
