...
// 상속해주는 클래스를 super / parent class 라고 함(상위 , 부모 , 조상 클래스)
// 상속받는 클래스를 sub / child class 라고 함(하위, 자식, 자손 클래스)

// 상속을 통해 코드의 중복을 제거하여 유지 보수성을 향상 시킬 수 있음.

// 상속을 하는 방법
// 접근제어자 class 자식 클래스명 extends 부모 클래스명 { 필드, ..., 메서드, ...}
...
class Human {
	...
	Human(String name,String address,String telNumber){
 	...
 	void print() {
		// super 키워드를 통해서 부모 클래스를 지칭할 수 있음.
...
class Police extends Human{
	...
	void print() {
		...
		//	System.out.println("telNumber:" + super.telNumber);
			super.print();
			System.out.println("position:" + this.classes);
		
	}
}
//class의 상속은 단일 상속만 지원함
//다중 상속을 허용했을 때 발생 가능한 다이아몬드 문제의 원인을 제거할 수 있음
//다이아몬드 문제: 두개 이상의 클래스를 상속 했을 때, 각 클래스 가 동일한 이름의
...
class Developer extends Human{
	...
	// 오버라이딩
	// 상위 메서드의 정의 되어 있는 메서드를 재정의 하는 것
	// 오버라이딩을 통해서 코드의 중복제거 / 메서드 유연성을 확보 할 수 있음.
	// 또한 오버라이딩을 통해서 다형성을 구현 할 수 있음
	
	// 오버라이딩의 조건
	// 메서드 이름, 반환 타입, 매개변수의 타입/ 갯수 / 조합이 상위클래스의 메서드와 같아야 함
	// 상위 클래스에서 final 로 정의된 메서드는 오버라이딩 할수 없음
	// 오버라이딩 메서드의 접근 제어자는 상위 클래스의 메서드 보다 좁게 변경할 수 없음
	
	// @Override : 컴파일러한테 아래에 오는 메서드는 상위클래스의 메서드를 오버라이딩 한것이라고 알려주는 것
	@Override
	void print() {
		System.out.println("name:" + this.name);
		System.out.println("address:" + this.address);
...
public class Oop3 {
	...
	public static void main(String[] args) {
		...
		System.out.println(otherPolice.address);
