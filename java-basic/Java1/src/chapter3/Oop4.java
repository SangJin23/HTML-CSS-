...
public class Oop4 {
	...
	public static void main(String[] args) {
		...
		// chapter3etc 패키지에 선언된 ExampleClass1 의 publicVariable 필드는
		// 접근 제어자가 public 이기 때문에 어떤 패키지에서도 접근이 가능
		exampleClass1.publicVariable = "public";
		// chapter3etc 패키지에 선언된 ExampleClass1 의 protectedVariable 필드는
		...
		// chapter3etc 패키지에 선언된 ExampleClass1 의 defaultVariable 필드는
		// 접근 제어자가 default 이기 떄믄에 외부패키지에서는 접근이 불가능
		...
		// chapter3etc 패키지에 선언된 ExampleClass1 의 privateVariable 필드는
		// 접근 제어자가 private 이기 때문에 외부 클래스에서는 접근이 불가능
		exampleClass1.privateVariabvle = "private";
		exampleClass2.privateVariabvle = "private";
		// 같은 패키지에 선언된 ExampleClass2의 privateVariable2 필드는
		// 접근 제어자가 private 이기 때문에 같은 패키지 내부라 해도 접근이 불가능
		exampleClass2.privateVariabvle2 = "private";
...
class ExampleClass2 extends ExampleClass1{
	...
	ExampleClass2 (){
		...
		// private으로 지정되어 있는 privateVariable은
		// 해당 클래스 내에서만 접근 가능
...
class User{
	...
	// 해당 클래스로 생성된 인스턴스의 필드 값을 무결성을 보장할 수 있음
	// 클래스의 구성 필드를 숨길수 있음
	private String name;
	private int age;
	...
	// 데이터를 초기화하기 위해 모든 필드의 값을 받는 생성자를 만들어 주어야한다.
	...
	// 특정 필드의 데이터를 반환하는 public 접근 제어자의 getter 메서드가 필요함.
	...
	// 다만, 빈 생성자를 통해서 인스턴스를 생성하고 추후에 초기화를 하는 인스턴스 이라면
	// setter 매서드가 반드시 필요함
...
class Board{
	...
	public String getBoardWriterld() {
		return boardWriterld;
	}
	
	public void setBoardWriterld(String boardWriterld) {
		this.boardWriterld = boardWriterld;
	}
	...
	public String toString() {
		return "Board [boardNumber=" + boardNumber + ", boardTitle=" + boardTitle + ", boardContent=" + boardContent
				+ ", boardWriteDatetime=" + boardWriteDatetime + ", boardWriterld=" + boardWriterld
